...
<c:if test="${loginUser.userId == 'admin' }">
    ...
    <div id="paging-area" align="center">
    	<c:choose>
    		...
    		<c:otherwise>
    			<button onclick="location.href='searchNotice.ad?cPage=${pi.currentPage - 1}&search=${search }&scitem=${scitem }'"><</button>
    		</c:otherwise>
    	</c:choose>
    	...
    	<c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage }">
	    	<c:choose>
	    		...
	    		<c:otherwise>
	    			<button onclick="location.href='searchNotice.ad?cPage=${p}&search=${search }&scitem=${scitem }'" >${p }</button>
	    		</c:otherwise>
	    	</c:choose>
    	</c:forEach>
    ...
    <script>
        ...
        $("tbody td").not($(".ck")).click(function(){
            ...
            location.href="adminNoticeDetailView.ad?bno="+bno;
        })
