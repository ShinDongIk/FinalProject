...
<body>
    ...
    <div id="paging-area" align="center">
    	<c:choose>
    		...
    		<c:otherwise>
    			<button onclick="location.href='searchFaq.ad?cPage=${pi.currentPage - 1}&search=${search }&scitem=${scitem }'"><</button>
    		</c:otherwise>
    	</c:choose>
    	...
    	<c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage }">
	    	<c:choose>
	    		...
	    		<c:otherwise>
	    			<button onclick="location.href='userSearchFaq.us?cPage=${p}&search=${search }&scitem=${scitem }'" >${p }</button>
	    		</c:otherwise>
	    	</c:choose>
    	</c:forEach>
    	...
    	<c:choose>
    		...
    		<c:otherwise>
    			<button onclick="location.href='userSearchFaq.us?cPage=${pi.currentPage + 1}&search=${search }&scitem=${scitem }'">></button>
    		</c:otherwise>
    	</c:choose>
    </div>
    <br>
    <script>
        $("tbody td").not($(".ck")).click(function(){
            ...
            location.href="userFaqDetailView.us?bno="+bno;
        })
    </script>
